...
```
After successfully loading the data and saving it in the matrices, I want to look at distribution of the location ratings. I collapse the data across all `r N` participants and create a histogram:
...
As it turns out, there is a significant positive correlation between the average ratings across locations and the overall ratingsr, *r* `r rValue(correlation$estimate)`, *p* `r pValue(correlation$p.value)`. 